...
The gate is the `multi_image_group` **capability**, not `adapter_id != pro`. Pro is the only family member without it today, so the two agree right now — and would keep agreeing while silently switching 组图 on for the next single-image variant, generating and billing images nobody asked for. `supports()` refuses and names the capable models from the registry; `build_payload` refuses again for direct callers.
