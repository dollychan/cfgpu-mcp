...
上行档位名（`1K` / `1.5K` / `2K`），并在 prompt 中用自然语言描述图片宽高比、图片形状
或图片用途，由模型判断生成图片大小。

> **cfgpu 不用这一种。** unified schema 的 `resolution` 是档位名，但 adapter 会把
> （`resolution`, `aspect_ratio`）查成一个精确像素对，始终以**方式 1** 上行。下表是这个
> 映射，不是上行的档位名。

| 档位 | 说明 |
|------|------|
| 1K | 映射成精确像素，对照表见下方 |
| 1.5K | 同上。官方称**与 1K 同价且效果更好**，优先于 1K |
| 2K | 同上（默认档位） |

> **三档都映射成像素**：unified schema 里 `aspect_ratio` 是必有的显式参数（默认 `1:1`），
> 只有像素能真正兑现它；发档位名等于把画幅交给模型从 prompt 里猜。
>
> 代价要知道：Pro 的价格在 1K 处分档（(0,1K] 0.4 元、(1K,∞] 0.8 元），而官方写着
> 1.5K 与 1K 同价——这个等式只有在**档位名本身是计价依据**时才成立（1.5K 的像素数
> 如 2048x1152 = 2,359,296，远在上一档区间里）。所以 1.5K 发像素**可能按 0.8 元计费**，
> 用它换的是一个确定的画幅。要按 1K 价拿 1.5K 效果，请改用 1K 档。

各档位映射的宽高像素值。**注意与 Seedream 5.0 lite / 4.5 / 4.0 的同名档位不是同一张
表**（如 2K 16:9：Pro 是 2816x1584，lite 是 2848x1600；1K 16:9：Pro 是 1424x800，
4.0 是 1280x720）。三档全部落在总像素范围 [921600, 4624220] 与宽高比范围 [1/16, 16] 内：
