...
按秒计价，`duration_seconds` 直接决定花费：2k / 15 秒一次约 8 元。
...
| `resolution` | `768p` \| `2k` | 省略时为 `768p`；**只有这两档**，`480p` / `720p` / `1080p` 都不支持 |
...
`768p` / `2k` 就是上游 MiniMax 的原生档位（`768P` / `2K`），本适配器只做大写，不做换算。
它们从前是靠「传 `720p` / `1080p`、内部翻译」实现的 —— 那意味着要 `720p` 的调用方被静默按
`768P` 出片和计费，而这两个真实档位反而没有拼法。现在两档是统一枚举里的一等取值；对应地，
向本模型传 `720p` 会**直接报错**并列出这两档，不再静默替换。省略 `resolution` 时用 `768p`
...
（显式传 `480p` / `720p` / `1080p`、超过 15 秒、首尾帧与参考素材混用等）自动让位给下一顺位
...
参数错误多为 `prompt` 为空、`duration_seconds` 越界、`resolution` 传了 `768p` / `2k` 以外的档位，
