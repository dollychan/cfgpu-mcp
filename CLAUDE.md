...
Pro's 1K and 1.5K are emitted as the **tier name** rather than pixels. Its price steps at 1K, and 1.5K is documented as billed at the 1K rate — which can only hold if the tier name is what billing reads, since 1.5K's pixel count sits well inside the upper band. Expanding them to exact pixels would double the price of the call to buy an exact aspect ratio the model is expected to infer from the prompt anyway; `aspect_ratio` is consequently advisory at those two tiers.
