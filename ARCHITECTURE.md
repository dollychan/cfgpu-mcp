...
`MiniMax-H3` 就是这么来的：它诚实的 speed 3 / cost 1（768P 0.1 元/秒，是全队按秒计费里最便宜的一档，2K 0.5）在 balanced 档记 2 分，与 `doubao-seedance-2-0-fast` / `cf-imagine-video` / `wan-video-fast` **打平而非胜出**，平局再落到 `auto_priority`，那里 `doubao-seedance-2-0-fast` 已经声明了 2。也就是说启发式无论如何都不会自己落到这里，于是路由决定写成路由决定。**反过来说，`cost_tier` 也不是这条路由的机关**：2026-09-07 实测把它在 1 和 2 之间来回改，fast/balanced/best × t2v/i2v/r2v/720p/2k/20s 的落点一个都不变 —— 它只是价格声明。（该档位曾长期写 2，源于把本模型的价格读成 0.4/0.8；`card.md` 的 0.1/0.5 与 成本档位 1/5 才是对的。）
...
**三个字段都随 `extends` 继承**，和其它 YAML 字段一样 —— 而 `gpt-image-2` 正是整条 nano-banana 链的父节点、`doubao-seedream-5-0-lite` 是 4.0 / 4.5 的父节点，所以不该继承的变体必须在自己的 YAML 里显式清零（`nano-banana-2` / `-pro-official` / `-pro-premium` 的 `quality_rank: 0`，`doubao-seedream-4-0` / `4-5` 的 `auto_priority: 0`，以及 `nano-banana-2` 的 `default_for: []` —— 这一处清零覆盖整条子树：`-pro` extends 它、`-pro-official` / `-pro-premium` 又 extends `-pro`，不清的话四个模型会一起声明自己是 balanced 默认，五个声明再落回分数决胜，正好退回 `default_for` 要取代的那种偶然次序，且全程不报错）。两个 Seedance 声明都落在 `wan-2-0` 链的叶子上（`2-0-fast` / `2-5` 无子节点），故无需清零 —— 但若日后有模型 `extends` 它们，这一条同样适用。漏掉一处不会报错，只会让一个没人提名的模型悄悄升上首选。`test_declared_ranks_are_not_inherited_down_the_extends_chain` 钉住这组解析后的值。
...
ModelAdapter (ABC, adapters/base.py)
    ...
    │       ├── GptImage2Adapter      gpt-image-2；resolution（1K→""）、quality_tier → quality
    │       └── NanoBananaAdapter     nano-banana-2 / nano-banana-pro（extends 链）；resolution → image_size
    │
...
**`label`（产物名字回显）**：第三个回显字段，机制与 `caption` 逐字节相同（同一个 `stamp_echo()`、保留键 `_label`、共用的 `label_field()`），区别只在它是给**人**看的展示名 / 文件名。**为什么不与 caption 合并成一个字段**：两者服务的读者不同——caption 落在 agent 每轮重读的台账里，职责是让模型分辨哪条素材是哪条，因而要能消歧；label 落在用户的素材面板里当文件名，因而要短、要稳、不含文件系统拒收的字符。长到能消歧的描述做文件名很糟，短到能显示的文件名消不了歧。更硬的一条是**归属不同**：宿主通常允许 agent 随时改写 caption，而 label 是素材的出生名——合并即意味着一次无关的模型调用能改掉用户面板里的文件名。两处取舍：①`LABEL_MAX_CHARS=100`，比 caption 短是故意的（这是文件名），且只做**纯截断**——非法字符替换、后缀保留这类清洗留给宿主，只有它知道自己的存储规则；②**两个字段互不兜底**，只传一个绝不派生另一个，回落策略（label → caption → object_key 基名）属于宿主。`public_payload()` 现在剥的是 `_ECHO_PAYLOAD_KEYS` 这一个元组而非手写清单——会增长的正是这份清单，漏一个就是内部键静默泄进所有调用方看到的 payload。
...
│   ├── kling_video.py          Kling Video O1 的 Python Adapter（flat prompt/size/mode/seconds/sound + image_list/video_list）；validation_corrections 把 with_audio 钉进 corrected_args
...
│   │   └── card.md
│   ├── cfgpu-minimax-h3/
│   │   ├── adapter.yaml        MiniMax-H3（provider: cfgpu，内建；逐请求令牌对它生效），resolutions: [768p, 2k] + default_resolution: 768p，4–15s
│   │   └── card.md
...
- `resolution`（视频）：统一枚举是 `480p / 720p / 768p / 1080p / 2k / 4k`，**默认值是 `None`，不是某一档**。**分辨率是逐模型的取值集合**：能查到官方取值范围的模型在 `adapter.yaml` 用 `resolutions: [...]` 声明（Seedance 2.5 / 2.0 fast / 2.0 mini 均为 `[480p, 720p]`，Seedance 2.0 为 `[480p, 720p, 1080p, 4k]`，MiniMax-H3 为 `[768p, 2k]`），由 `ModelAdapter.supports()` 统一校验；未声明（`None`）= 不做本地限制，即该字段引入前的行为。否则上游会回 `the parameter resolution specified in the request is not valid for model X in i2v`。`1080p` 由 WAN 2.0 / Doubao Seedance 2.0 / Doubao Seedance 1.5 Pro / HappyHorse 支持（`happyhorse` 在 `build_payload` 中 `.upper()` 成 `1080P`；`happyhorse` 仍拒绝 `480p`）。**另一例外：WAN 2.0 Fast 文生视频（t2v）不支持 `1080p`，`supports()` 会拒绝（仅 480p/720p；带首帧/参考图视频的 i2v 场景才放行 1080p）；`model="auto"` 命中该组合时会自动回退到完整版 WAN 2.0。**

- `with_audio`：统一开关，但**只有四个族真的会发它** —— 可灵（`sound: on/off`）、Seedance（`generate_audio`）、`cfdream/minimax-h3*`（`with_audio`）；万相 2.6/2.7 与 HappyHorse 压根不发，Grok 与 `MiniMax-H3` 恒出音频、没有开关。**可灵额外把它钉进 `corrected_args`**（`KlingVideoAdapter.validation_corrections`，2026-09-07）：它的 card 里 `sound` 的默认值写的是「-」，即上游没有默认值，于是统一 Schema 那个 `with_audio: bool = True` 就是全部决定 —— 而它是静默的。按调用方原始参数生成的审批卡上没有音频这一行，人却批准了一条自带配乐的片子。所以这是**钉住（pin）而非回退**，与 `model="auto"` 解析后回钉具体模型同形、同理由：审批只有在指明「到底会跑什么」时才有意义。**无条件钉**，因为它必须无条件：`with_audio` 是带具体默认值的裸 `bool`，传了 `True` 的调用方和什么都没传的调用方到这里完全一样，「只在缺省时补」根本不可表达，而猜错的那一半恰恰是唯一需要补的那一半。范围也是刻意只给可灵的 —— Seedance 同样认这个字段，要不要一并钉是逐个 card 该做的判断，不是改一个 adapter 的副作用（`test_only_kling_pins_with_audio` 钉住这条边界）。

  **默认值为什么是 `None`（`default_resolution`，2026-09-07）。** `resolution` 是 `supports()` 会读的字段，所以一个写死的全队默认值等于「每个模型都必须支持这一档，否则会被一个调用方根本没提的参数挤出 `model="auto"`」。MiniMax-H3 恰好是这种模型：它只有 768P / 2K 两档，压根没有 720p。于是统一 Schema 的默认值改成 `None`（= 「你来定」），真实档位落在 `adapter.yaml` 的 `default_resolution`（缺省 `720p`，与改动前完全一致，只有 MiniMax-H3 写 `768p`），由 `ModelAdapter.resolve_resolution(req)` 解析 —— 与 `duration_seconds` → `default_duration_seconds` 完全同构。**所有 adapter 读的都是 `self.resolve_resolution(req)`，不是 `req.resolution`。** 不这么做的代价不是抽象缺失，而是最朴素的一次调用 `generate_video(prompt=...)` 会绕开 `MiniMax-H3` 的 `default_for: [balanced]` 声明，而显式点名 `model="MiniMax-H3"` 又会直接报「不支持 720p」—— 和 `_T2V_DEFAULT_RATIO` 当初躲开的是同一个坑。相应地 `validation_corrections` 对 `resolution is None` 返回 `{}`：那已经是该模型自己的档位，把它写进 `corrected_args` 等于把一个「交给模型决定」钉成调用方从没做过的选择。

  **`768p` / `2k` 是 MiniMax-H3 独有的一等取值，不是别名。** 它们从前是靠「声明 720p/1080p，再在 `build_payload` 里翻译成 768P/2K」实现的，结果是：要 720p 的调用方被静默按 768P 出片和计费（0.1 元/秒，2K 则是 0.5），而这个模型真正提供的两档反而没有拼法。现在 adapter 只做 `.upper()`，翻译表已删除。其他模型**不**接受这两档 —— `supports()` 拒绝，`validate_only` 通过 `validation_corrections` 回退到「不高于请求值的最近一档」（`768p` → `720p`，`2k` → `1080p`，Seedance 2.0 fast 这种没有 1080p 的则 `2k` → `720p`）。档位次序表是 `adapters/base.py` 的 `_RESOLUTION_ORDER`，**必须与统一枚举逐一对应**：漏一个不会报错，只会让那一档静默跳过回退，`validate_only` 于是放行一个正式调用必然拒绝的请求（`test_resolution_order_covers_the_whole_schema_enum` 钉住这一点）。
