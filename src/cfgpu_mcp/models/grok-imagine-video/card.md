...
与同家族的 [Grok Imagine Video 1.5](../grok-imagine-video-1-5/card.md) **API 形状完全一致**（同一个 `GrokVideoAdapter`），差别只在价格档位：本模型更便宜，且 480P 以上不再细分（1.5 在 720P 以上另有一档）。
...
按秒计费，单价随输出分辨率分档（两档，480P 以上统一价）：

| 条件 | 计费项 | 价格 |
|------|--------|------|
| 分辨率 (0, 480P] | 统一计价 | 0.275 元 / 秒 |
| 分辨率 (480P, 无限] | 统一计价 | 0.385 元 / 秒 |

## 参数说明

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| model | string | ✓ | - | 固定值：`grok-imagine-video` |
...
按秒计费、单价随分辨率分档，但任务响应里**没有 `usage` 对象**，计费数值散在 `data` 里。adapter 将其组装为与可灵 / 万相 / HappyHorse 一致的形状：
