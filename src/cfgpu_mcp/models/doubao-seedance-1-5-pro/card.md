...
| 场景 | 单价 |
|------|------|
| 有声视频（有/无视频输入） | 0.0168 元 / K tokens |
| 无声视频（有/无视频输入） | 0.0084 元 / K tokens |

> 计费由是否生成音频决定，与是否传入参考视频无关。Token 消耗见响应中的 `usage.totalTokens`。
...
- 需要多模态参考/视频编辑/延长 → 使用 `wan-video`
- 追求最高质量全功能 → 使用 `wan-video`
...
| ratio | string | - | adaptive | 宽高比（与 wan-video 一致） |
...
以下 `wan-video` 支持的输入类型在 1.5 Pro 中**不可使用**：
