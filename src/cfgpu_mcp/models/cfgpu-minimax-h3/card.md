...
|---|---|---|
| `480p` | 输入视频 | 0.1 元 / 秒 |
| `480p` | 输入图片 | 0.04 元 / 张（前 5 张免费） |
| `480p` | 输出视频 | 0.1 元 / 秒 |
| `768p` | 输入视频 | 0.2 元 / 秒 |
| `768p` | 输入图片 | 0.08 元 / 张（前 5 张免费） |
| `768p` | 输出视频 | 0.2 元 / 秒 |
| `2k` | 输入视频 | 0.5 元 / 秒 |
| `2k` | 输入图片 | 0.1 元 / 张（前 5 张免费） |
| `2k` | 输出视频 | 0.5 元 / 秒 |

输入、输出视频均按秒计费；输入图片每次请求前 5 张免费。`duration_seconds` 直接决定视频费用：2k / 15 秒的输出视频费用为 7.5 元。
...
| `resolution` | `480p` \| `768p` \| `2k` | 省略时为 `768p`；`720p` / `1080p` 不支持 |
...
它们从前是靠「传 `720p` / `1080p`、内部翻译」实现的 —— 那意味着要 `720p` 的调用方被静默按
`768P` 出片和计费，而这些真实档位反而没有拼法。现在三档是统一枚举里的一等取值；对应地，
向本模型传 `720p` 会**直接报错**并列出支持档位，不再静默替换。省略 `resolution` 时用 `768p`
（`adapter.yaml` 的 `default_resolution`），所以最朴素的 `generate_video(prompt=...)` 依然
路由得到本模型 —— 统一 Schema 的 `resolution` 默认值是 `None`（「你来定」），不是某一档。
...
（显式传 `720p` / `1080p`、超过 15 秒、首尾帧与参考素材混用等）自动让位给下一顺位
模型。**不传 `resolution` 时不会因此出局**：那时用本模型自己的 `768p`。
...
参数错误多为 `prompt` 为空、`duration_seconds` 越界、`resolution` 传了 `480p` / `768p` / `2k` 以外的档位，
