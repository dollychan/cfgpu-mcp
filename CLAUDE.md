...
`adapters/regions.py` is not an adapter — it is the shared prompt-embedded region dialect (grid conversion, `[[...]]` placeholder resolution, the refusal text) used by `seedream` and `vision_chat`, and by `ModelAdapter.supports()` for the capability gate.
...
**Normalised floats, not the [0,999] grid one of the dialects speaks.** Both current consumers embed coordinates in the prompt text on a 1000-cell grid, but routing the shared schema through that grid would tax every *other* dialect with one model's quantisation (±4px systematic on a 4000px image, incurred for a model that may not even be selected) and would force an inclusive-last-cell rule into the shared layer. `[0,1] → [0,w]` needs no such rule. `adapters/regions.py` owns the single conversion — `floor` on the near edge, `ceil()-1` on the far edge, so `[0,0,1,1]` reaches `[0,0,999,999]` rather than stopping a cell short and drifting further on each edit round-trip. Both adapters call it; two copies would be fixed one at a time.
